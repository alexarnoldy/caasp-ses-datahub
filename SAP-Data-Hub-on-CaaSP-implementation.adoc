...
TIP: Use the command `ceph auth ls | grep data-hub-infra` to ensure the user hasn't already been created. If it has, skip this step and continue on to "Gather the keys for the SES admin and data-hub-demo users"
...
* Run the following command from each physical host:
----
...
NOTE: Run the following eight commands on each physical host to mount the new RBD image and cause it to remount during system boot
