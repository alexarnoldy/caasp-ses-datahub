...
*** Launch nginx container: docker run --name mynginx  -v /home/sles/autoyast_templates:/usr/share/nginx/html:ro -P -d nginx:latest
**** Take note of the network port assigned to nginx
*** Test that the master autoyast file is available: `curl http://admin.caaspv4.com:<nginx port>/master.xml`
** sudo usermod -aG docker sles ; sudo su - sles

** Setting up the autoinst.xml file for the cluster nodes
