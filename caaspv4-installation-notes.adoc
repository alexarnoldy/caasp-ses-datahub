...
////
Need to change these to sed -i statements
////
...
** Provide the SLES 15 SP1 DVD1 installer DVD or ISO to the VM or host BIOS
** On Installation line: `autoyast=http://admin.caaspv4.com:<nginx port>/<worker node name>.xml ifcfg=eth0=<IP of worker node>/24,<IP of gateway>,<IP of DNS server,<search domain> hostname=<worker node name>.caaspv4.com
