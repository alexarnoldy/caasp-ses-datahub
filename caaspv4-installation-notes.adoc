...
**** CAUTION: If you choose to enable swap for the Management Workstation, you MUST remove it from the AutoYaST clone file for the CaaS Platform nodes
* Create New User:
...
* On Boot Options line: `autoyast=http://admin.caaspv4.com:<nginx port>/master.xml ifcfg=eth0=<IP of master>/24,<IP of gateway>,<IP of DNS server>,<search domain> hostname=master.caaspv4.com

.After Master Node completes installation, Adjust its networking to suit environment. 
* In this document we are creating a bonded network from eth0
...
* Ensure "caasp","version":"4.0", "SLES","version":"15.1", and "sle-module-containers","version":"15.1" are registered
** `for EACH in master worker1 worker2 worker3; do echo $EACH; ssh $EACH sudo SUSEConnect -s | egrep --color "caasp|SLES|containers|\"Registered\"" && echo"" && echo "Press Enter for next system" && read NEXT; done`
...
** Note this command bootstraps the CaaS Platform cluster with master.caaspv4.com as the first (in this case, only) master node. Internally, Kubernetes will know this node as "master"
