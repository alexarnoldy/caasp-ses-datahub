...
* Resister SLES 15 SP1, CaaS Platfrom version 4.0.0, and the Containers Module: for EACH in master worker1 worker2 worker3; do echo $EACH; ssh $EACH sudo SUSEConnect -r $REGCODE && sudo SUSEConnect -p sle-module-containers/15.1/x86_64 && sudo SUSEConnect -p caasp/4.0/x86_64 -r $REGCODE; echo ""; done
* Ensure correct products are registered
** `for EACH in master worker1 worker2 worker3; do echo $EACH; ssh $EACH sudo SUSEConnect -s; echo ""; done`
