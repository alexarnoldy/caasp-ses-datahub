...
////
Need to update the caaspv4 file with the changes in this part
////
.Finish preparing the Administrative Workstation:
* Enabled passwordless sudo for the user geeko
** `sudo bash -c "echo 'geeko ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers.d/01geeko"`
* The eaisest way to insert the Administrative Workstation's SSH key into the AutoYaST clone file is to add it to the Administrative Workstation before creating the clone file
** `ssh-keygen`
** `cat ~/.ssh/id_rsa.pub >> ~/.ssh/authorized_keys`
* Update the NTP client settings. This example simply accepts the default, public NTP servers for the region. Select a different, local NTP if available, just make sure the Administrative Workstation can sync with it before moving forward.
** `sudo yast timezone`
*** `other Settings`
