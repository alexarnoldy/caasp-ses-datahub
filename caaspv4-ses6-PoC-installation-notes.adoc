...
* `export RGWUser="data-hub-demo" && echo ${RGWUser}`
...
NOTE: Take note of the access_key and secret_key. They will be needed to access the S3 gateway. The command `radosgw-admin user info --uid=${RGWUser} | grep -A1 access_key` can be used to retrieve the keys at any time.

.Take note of the DNS FQDN and port for the Rados Gateway

`grep rgw /etc/ceph/ceph.conf`
* The "rgw frontends" line shoud include `port=` to indicate if the gateway must be access with HTTP (port=80), or HTTPS (port=443)

.Install the s3cmd utility on the SES6 Administrative Workstation or another SLES/openSUSE system external to the cluster
----
----
 
////
Doesn't work if you don't have the browser running on the target system
** Go to `https://software.opensuse.org/package/s3cmd?search_term=s3cmd`
** Download the s3cmd.ymp file for the appropriate O/S and distribution from either experimental packages or community packages
////

NOTE: The s3cmd utility is not supported by SUSE and thus cannot be guaranteed to be free of defects and/or malware. Use caution when installing non-supported software into a production or other higly valuable environment.

* Ensure the python3-pip package is installed, `sudo zypper info python3-pip`, if not install it: `sudo zypper -n install python3-pip`
* Install s3cmd: `sudo pip install s3cmd`

////
This is the method to get s3cmd as a static utility
wget -O s3cmd-latest.zip https://sourceforge.net/projects/s3tools/files/latest/download
unzip s3cmd-latest.zip
cd s3cmd-?.*
"Move "s3cmd" file and "S3" subdirectory to some other path. Make sure that "S3" subdirectory ends up in the same place where you move the "s3cmd" file. "
////

.Use the s3cmd utility to access the S3 gateway
* Run the `s3cmd --configure` operation with the following options:
** Provide the `Access Key` and `Secret Key`, without quotes, for the data-hub-demo user
** Press Enter to skip the `Default Region` option
** Provide the FQDN for the Rados Gateway for the `S3 Endpoint` option
** Enter `%(bucket)s.` and the FQDN for the Rados Gateway for the `DNS-style bucket+hostname`
** Press Enter to skip the `Encryption password` and `Path to GPG program` options
** Enter "No" for the `Use HTTPS protocol` option if "rgw frontends" in the ceph.confg file referred to port 80
** Press Enter to skip the `HTTP Proxy server name` option
** Enter "y" to `Test access with supplied credentials?`
** Enter "y" to `Save settings?`, even if the credential test failed.

CAUTION: If the credential test failed, it could be due to specifying HTTP when the gateway will only accept HTTPS, or that the Access Key and or Secret Key are incorrect. This test does not verify that DNS is set up correctly to resolve bucket names to the Rados Gateway. 

.Ensure DNS resolution for buckets in the Rados Gateway
* Check name resolution with the `getent hosts` command by adding any name to the beginning of the Rados Gateway's FQDN
** For example, if the Rados Gateway is `rados-gateway.ses.lab`, execute the command `getent hosts vora-checkpoint-store.rados-gateway.ses.lab`
*** This should resolve to the same IP address as `rados-gateway.ses.lab`

NOTE: An example of the entry needed in the DNS server's /var/lib/named/master/<zone> file to resolve bucket names would be `*.rados-gateway         IN CNAME        rados-gateway.ses.lab.`   Take note of the trailing dot in the value field and that named.service must be restarted after manually adding the entry.

.Create the bucket for the Vora checkpoint store

* `s3cmd mb s3://vora-checkpoint-store`
** The command should respond with Bucket created
* Create a test file to put into the bucket: `echo test > test-file.txt`
* Put the file into the vora-checkpoint-store bucket: `s3cmd put test-file.txt s3://vora-checkpoint-store`
* Verify the file is in the bucket: `s3cmd ls s3://vora-checkpoint-store`
